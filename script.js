...
function showPredictions(repositories) {
  ...
  for (let repoIndex = 0; repoIndex < 5; repoIndex++) {
    let name = repositories.items[repoIndex].name;
    let owner = repositories.items[repoIndex].owner.login;
    let stars = repositories.items[repoIndex].stargazers_count;
